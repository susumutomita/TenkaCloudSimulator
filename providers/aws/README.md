...
SSM Session Manager の session ID、stream URL、期限 transition、stream registry は target identity を
含みます。data channel が実行する `SendCommand` / `TerminateSession` も開始時の targetId を維持し、
同じ deployment 内の別 AWS target や `default` target へ session state を付け替えません。
