...
```text
provider / service / resourceType / operation / fidelity
```
...
返し、初期 consumer が unknown field を無視できる形にします。
...
2 から 8 個の target を持ちます。全 target の
...
復元します。既存 world への上書き import は行いません。
...
- `covered`: 必要な fidelity 以上を実装済みである。
- `missing`: resource または operation がありません。
- `insufficient`: operation はあるが fidelity が不足している。
